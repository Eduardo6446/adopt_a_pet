...
def profile(request, username):
    ...
    #pets = Pet.objects.get(owner=request)
...
def search_view(request):
    ...
    if busqueda:
        ...
        pets = Pet.objects.filter(
            ...
            Q(breed__icontains = busqueda)
        ).distinct()
         
    return render(request, 'Pets/search.html', {'mascotas':pets, 'post': post, "suggestions": suggestions})
...
def create_pet(request):
    ...
    context = {}
    if request.method == "POST":
        form = formApregarMascota(request.POST, request.FILES)
        if form.is_valid():
            # nom = form.cleaned_data.get("name")
            # ag = form.cleaned_data.get("age")
            # bree = form.cleaned_data.get("breed")
            # specie = form.cleaned_data.get("species")
            # description = form.cleaned_data.get("description")
            # owne = form.cleaned_data.get("owner")
            # foto = form.cleaned_data.get("pet_pic")
            # obj = Pet.objects.create(
            #     name = nom,
            #     age = ag,
            #     breed = bree,
            #     species = specie,
            ...
            mascota.owner = request.user

            mascota.save()

            return HttpResponseRedirect(reverse('index'))
    else:
        form = formApregarMascota()
    context['form'] = form
            
    return render(request,"Pets/addPet.html",context)
