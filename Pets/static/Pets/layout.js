...
function createpost() {
    ...
    popup.querySelector('.large-popup').querySelector("#post-text").addEventListener('input', (event) => {
        ...
        if(event.target.value.trim().length > 0 && event.target.parentElement.querySelector('#id_owner').value.trim().length > 0) {
            popup.querySelector('.submit-btn').disabled = false;
        }
    });
}
...
function edit_post_submit(post_id) {
    ...
    .then(response => {
        if(response.success) {
            ...
            console.log('Hubo un error al editar la publicación :/');
        }
    });
    remove_popup();
    return false;
...
function login_popup(action) {
    ...
    if(action === 'like') {
        ...
        document.querySelector('.main_text-div').querySelector('h2').innerText = 'Dale me gusta a una publicación';
    }
        ...
        document.querySelector('.main_text-div').querySelector('h2').innerText = 'Comenta para pedir más información';
    }
    else if(action === 'save') {
        document.querySelector('.icon-div').innerHTML = `
        ...
        document.querySelector('.main_text-div').querySelector('h2').innerText = 'Salva este post para verlo luego';
    }
        ...
        document.querySelector('.main_text-div').querySelector('h2').innerText = 'Siguelos para adoptar mas mascotas';
    }
}
